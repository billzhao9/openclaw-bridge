...
const bridgePlugin = {
    ...
    register(api) {
        ...
        function extractChannelsFromConfig() {
            const candidates = [
            ...
            if (!configPath) {
                api.logger.warn("[bridge] init-time channel extraction: no config file found");
                return [];
            }
            try {
                const raw = readFileSync(configPath, "utf-8");
                ...
                if (result.length > 0)
                    api.logger.info(`[bridge] init-time channel extraction: ${result.length} channels found`);
                return result;
            }
            catch {
                return [];
        ...
        function detectDiscordIdFromConfig() {
            const candidates = [
                process.env.OPENCLAW_CONFIG_PATH,
                process.env.OPENCLAW_HOME ? join(process.env.OPENCLAW_HOME, "openclaw.json") : "",
                join(homedir(), ".openclaw", "openclaw.json"),
            ...
            const configPath = candidates.find(p => existsSync(p));
            if (!configPath) {
                api.logger.warn("[bridge] detectDiscordId: no config file found from candidates: " + candidates.join(", "));
                return null;
            }
            try {
                const raw = readFileSync(configPath, "utf-8");
                ...
                const accounts = cfg.channels?.discord?.accounts;
                if (!accounts) {
                    api.logger.warn("[bridge] detectDiscordId: no discord accounts in config");
                    return null;
                }
                for (const account of Object.values(accounts)) {
                    if (account.token) {
                ...
                api.logger.error(`[bridge] detectDiscordId error: ${err.message}`);
            }
            return null;
        }
        /**
         ...
         */
        async function askPm(type, data, timeoutMs = 20_000, pmAgentId = "pm") {
                ...
                if (config.messageRelay) {
                    ...
                    if (config.isProjectManager) {
                        ...
                        relayClient.on('task_update', async (msg) => {
                            try {
                                const bumped = projectMgr.incrementRounds(msg.projectId, msg.taskId);
                                if (!bumped)
                                    return replyErr(msg, 'task_update_reply', "Project or task not found");
                                const { task, hardLimit } = bumped;
                                const project = projectMgr.readProject(msg.projectId);
                                if (project?.threadId && discordApi.isAvailable) {
                                    ...
                                    if (hardLimit) {
                                        projectMgr.updateTaskStatus(msg.projectId, msg.taskId, "blocked", { blockType: "stalled", blockReason: `Task exceeded hard round limit (${task.rounds})` });
                                        await discordApi.sendMessage(project.threadId, `⛔ **${task.title}** auto-blocked — hit hard round limit (${task.rounds}). Worker should stop and PM must intervene.`);
                                    }
                                }
                                replyOk(msg, 'task_update_reply', { posted: !!project?.threadId, rounds: task.rounds, hardLimitHit: !!hardLimit });
                            }
                            catch (err) {
                                replyErr(msg, 'task_update_reply', err.message);
                            }
                        });
                        // Task complete: update task status + outputs
                        relayClient.on('task_complete', async (msg) => {
                            try {
                                const task = projectMgr.updateTaskStatus(msg.projectId, msg.taskId, "completed", { outputs: msg.outputPaths || [] });
                                if (!task)
                                    return replyErr(msg, 'task_complete_reply', "Project or task not found");
                                ...
                                if (project) {
                                    const readyTasks = projectMgr.getReadyTasks(msg.projectId);
                                    for (const nextTask of readyTasks) {
                                        ...
                                        projectMgr.updateTaskStatus(msg.projectId, nextTask.id, "in_progress");
                                    ...
                                    const updatedProject = projectMgr.readProject(msg.projectId);
                                    if (updatedProject && updatedProject.tasks.every(t => t.status === "completed")) {
                                        ...
                                        if (updatedProject.threadId && discordApi.isAvailable) {
                                            ...
                                            await discordApi.sendMessage(updatedProject.threadId, `🎉 **Project Complete: ${updatedProject.name}**\n\n${deliverableList}\n\nAll deliverables are in the project directory.`);
                                        }
                                        ...
                                        if (discordChannel && discordApi.isAvailable) {
                                            const userMention = updatedProject.creatorUserId ? `<@${updatedProject.creatorUserId}>` : "User";
                                            await discordApi.sendMessage(discordChannel.channelId, `🎉 ${userMention} — **${updatedProject.name}** is complete! All deliverables are posted in the project thread.`);
                                        }
                                    }
                                }
                            }
                            catch (err) {
                                replyErr(msg, 'task_complete_reply', err.message);
                            }
                        });
                        // Task blocked
                        relayClient.on('task_blocked', async (msg) => {
                            try {
                                const task = projectMgr.updateTaskStatus(msg.projectId, msg.taskId, "blocked", { blockType: msg.blockType, blockReason: msg.reason });
                                if (!task)
                                    return replyErr(msg, 'task_blocked_reply', "Project or task not found");
                                const project = projectMgr.readProject(msg.projectId);
                                if (project?.threadId && discordApi.isAvailable) {
                                    const agents = await discoverAll(registry, offlineThresholdMs);
                                    const agentName = agents.find(a => a.agentId === msg.from)?.agentName || msg.from;
                                ...
                                replyErr(msg, 'task_blocked_reply', err.message);
                            }
                        });
                        // Query project(s)
                        relayClient.on('query_project', async (msg) => {
                            try {
                                if (msg.projectId) {
                                    const project = projectMgr.readProject(msg.projectId);
                                    if (!project)
                                        return replyErr(msg, 'query_project_reply', `Project "${msg.projectId}" not found`);
                                    return replyOk(msg, 'query_project_reply', { project });
                                }
                                replyOk(msg, 'query_project_reply', { projects: projectMgr.listProjects() });
                            }
                            catch (err) {
                                replyErr(msg, 'query_project_reply', err.message);
                            }
                        });
                        // Query assets for a project
                        relayClient.on('query_assets', async (msg) => {
                            try {
                                const assets = projectMgr.listAssets(msg.projectId, msg.type, msg.agent);
                                replyOk(msg, 'query_assets_reply', { assets });
                            }
                            catch (err) {
                                replyErr(msg, 'query_assets_reply', err.message);
                            }
                        });
                        // Query single asset by id or latest of a type
                        relayClient.on('query_asset', async (msg) => {
                            try {
                                const assets = projectMgr.listAssets(msg.projectId);
                                let asset;
                                if (msg.assetId)
                                    asset = assets.find(a => a.id === msg.assetId);
                                else if (msg.assetType)
                                    asset = assets.filter(a => a.type === msg.assetType).pop();
                                if (!asset)
                                    return replyErr(msg, 'query_asset_reply', "Asset not found");
                                const fullPath = join(projectMgr.getProjectDir(msg.projectId), asset.path);
                                replyOk(msg, 'query_asset_reply', { asset: { ...asset, fullPath } });
                            }
                            catch (err) {
                                replyErr(msg, 'query_asset_reply', err.message);
                            }
                        });
                        relayClient.on('register_asset', async (msg) => {
                            ...
                            try {
                                const inboxPath = join(workspacePath, "_inbox", msg.from, msg.filename);
                                if (!existsSync(inboxPath)) {
                                    throw new Error(`inbox file not found: ${inboxPath}`);
                                }
                                const asset = projectMgr.publishAsset(msg.projectId, inboxPath, msg.assetType || "deliverable", msg.description || "", msg.from, msg.taskId || "");
                                if (!asset) {
                                    relayClient.send({
                                ...
                                if (msg.taskId) {
                                    projectMgr.updateTaskStatus(msg.projectId, msg.taskId, "completed");
                                }
                                relayClient.send({
                                    type: 'register_asset_reply',
                                    replyTo: msg.id,
                                    ...
                                    for (const file of files) {
                                        ...
                                        for (const project of activeProjects) {
                                            ...
                                            if (directMatch || keywordMatch) {
                                                ...
                                                if (lower.includes('brief') || lower.includes('direction'))
                                                    assetType = 'brief';
                                                else if (lower.includes('script') || lower.includes('narration'))
                                                    assetType = 'script';
                                                else if (lower.includes('outline') || lower.includes('plan'))
                                                    assetType = 'outline';
                                                else if (lower.includes('storyboard'))
                                                    assetType = 'storyboard';
                                                else if (lower.includes('asset_list') || lower.includes('assets'))
                                                    assetType = 'asset-list';
                                                else if (lower.match(/\.(jpg|jpeg|png|webp)$/))
                                                    assetType = 'img';
                                                else if (lower.match(/\.(mp4|mov|webm)$/))
                                                    assetType = 'video';
                                                const asset = projectMgr.publishAsset(project.id, filePath, assetType, `Auto-organized from inbox/${agentDir}/${file}`, agentDir, // producer = agent directory name
                                                '');
                                                if (asset) {
                                                    ...
                                                    catch { /* file may have been moved by publishAsset */ }
                                                    matched = true;
                                                    break;
                                                }
                                            }
                        ...
                        setInterval(async () => {
                            try {
                                ...
                                for (const agent of agents) {
                                    if (agent.agentId === config.agentId)
                                        continue; // skip self
                                    ...
                                    for (const line of lines) {
                                        try {
                                            ...
                                            if (msg.type === 'task_complete') {
                                                projectMgr.updateTaskStatus(msg.projectId, msg.taskId, "completed", { outputs: msg.outputPaths || [] });
                                            }
                                            else if (msg.type === 'task_update') {
                                                projectMgr.incrementRounds(msg.projectId, msg.taskId);
                                            }
                                            else if (msg.type === 'task_blocked') {
                                                projectMgr.updateTaskStatus(msg.projectId, msg.taskId, "blocked", { blockType: msg.blockType, blockReason: msg.reason });
                                            }
                                            else if (msg.type === 'register_asset') {
                                                const inboxPath = join(workspacePath, "_inbox", msg.from, msg.filename);
                                                if (existsSync(inboxPath)) {
                                                    projectMgr.publishAsset(msg.projectId, inboxPath, msg.assetType || "deliverable", msg.description || "", msg.from, msg.taskId || "");
                                                    if (msg.taskId) {
                                                        projectMgr.updateTaskStatus(msg.projectId, msg.taskId, "completed");
                                                    }
                                                }
...
5. 不要自己回答用户的问题，所有内容都转发给 ${session.currentAgentName}
        ...
        api.registerTool({
            ...
            async execute(_id, params) {
                ...
                const isPmTarget = params.targetAgentId === "pm";
                if (isPmTarget && relayClient?.isConnected) {
                    ...
                    let assetType = params.assetType || "deliverable";
                    if (!params.assetType) {
                        const lower = localPath.toLowerCase();
                        if (lower.includes("/_briefs/") || lower.includes("\\_briefs\\"))
                            assetType = "brief";
                        else if (lower.includes("storyboard"))
                            assetType = "storyboard";
                        ...
                        if (reply.ok && discordApi.isAvailable) {
                            try {
                                ...
                                if (threadId) {
                                    ...
                                    await discordApi.sendMessageWithFile(threadId, join(workspacePath, localPath), `📎 **File from ${config.agentName}**: ${displayName}`);
                                }
                            }
        ...
        api.registerTool({
            ...
            async execute(_id, params) {
                ...
                if (params.projectId) {
                    const existing = projectMgr.readProject(params.projectId);
                ...
                for (const a of hits)
                    if (a.discordId)
                        toAdd.add(a.discordId);
                if (params.creatorUserId)
                    toAdd.add(params.creatorUserId);
                const added = [];
                for (const uid of toAdd) {
                    if (await discordApi.addThreadMember(thread.id, uid))
                        added.push(uid);
                }
        ...
        api.registerTool({
            ...
            async execute(_id, params) {
                ...
                if (params.creatorUserId) {
                    project.creatorUserId = params.creatorUserId;
                    projectMgr.writeProject(project);
                }
                const result = {
                    ...
                    if (discordChannel) {
                        try {
                            ...
                            if (agentIds.length > 0) {
                                ...
                                for (const a of hits) {
                                    if (a.discordId)
                                        agentDiscordIds.push(a.discordId);
                                }
                                mentions = hits
                                    .map(a => a.discordId ? `<@${a.discordId}>` : a.agentName)
                                    .join(" ") || mentions;
                            }
                            const thread = await discordApi.createThread(discordChannel.channelId, `🎬 ${params.name} — PM Managed`.substring(0, 100), `📋 **Project: ${params.name}**\n\n${params.description}\n\nTeam: ${mentions}\n\nProject thread created. All updates will be posted here.`);
                            ...
                            const added = [];
                            const toAdd = new Set(agentDiscordIds);
                            if (params.creatorUserId)
                                toAdd.add(params.creatorUserId);
                            for (const uid of toAdd) {
                                if (await discordApi.addThreadMember(thread.id, uid))
        ...
        api.registerTool({
            ...
            async execute(_id, params) {
                ...
                const pmId = params.pmAgentId || "pm";
                const pm = await registry.findAgent(pmId, offlineThresholdMs);
                if (!pm)
                    return { error: `PM agent "${pmId}" not found online. Use bridge_discover to see available PMs.` };
                // Step 1: send the file to PM's _inbox/<myAgentId>/
                const sendResult = await fileOps.sendFile(pm, params.filePath);
                if (!sendResult.delivered)
                    return { error: "File transfer to PM failed" };
                const filename = sendResult.filename ?? params.filePath.split(/[\\/]/).pop();
                // Step 2: send register_asset message and wait for PM to process
                if (!relayClient?.isConnected)
                    return { error: "Message relay not connected; cannot notify PM" };
                ...
                try {
                    ...
                    if (!reply.ok)
                        return { error: `PM rejected asset: ${reply.error}` };
                    // Auto-post file to project thread for Discord visibility
                    if (discordApi.isAvailable) {
                        try {
                            ...
                            const threadId = projectReply?.project?.threadId;
                            if (threadId) {
                                const displayName = params.filePath.split(/[\\/]/).pop() || "deliverable";
                                await discordApi.sendMessageWithFile(threadId, join(workspacePath, params.filePath), `📎 **Deliverable from ${config.agentName}**: ${displayName}\n${params.description || ""}`);
                            }
                        }
                        catch { /* auto-post is best-effort */ }
                    }
