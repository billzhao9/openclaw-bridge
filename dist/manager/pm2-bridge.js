...
export async function getProcessLogs(name) {
    ...
    try {
        ...
        const hasPM2Process = apps.some((a) => a.name === name);
        if (hasPM2Process) {
            ...
            const lines = stdout.split("\n").filter((l) => !l.startsWith("[TAILING]") && l.trim());
            if (lines.length > 0)
                return lines.join("\n");
        }
    }
    ...
    if (IS_MAC) {
        try {
            ...
            if (!existsSync(logDir))
                return "(logs unavailable — no log directory)";
