...
export class DiscordApi {
  ...
  loadToken(): void {
    ...
    for (const configPath of configCandidates) {
      ...
      try {
        ...
        for (const acc of Object.values(accounts) as Array<{ token?: string }>) {
          if (acc.token) {
   ...
   */
  async createThread(channelId: string, name: string, message?: string): Promise<{ id: string; name: string }> {
