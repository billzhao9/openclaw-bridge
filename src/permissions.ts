...
export function checkPermission(action: string, config: BridgeConfig): boolean {
  ...
  if (PM_ONLY_ACTIONS.has(action)) return config.isProjectManager === true;
...
export function assertPermission(action: string, config: BridgeConfig): void {
  if (!checkPermission(action, config)) {
    if (PM_ONLY_ACTIONS.has(action)) {
      throw new Error(
        `STOP — "${action}" is PM-only. You are NOT the PM. Use bridge_submit_deliverable or bridge_send_file to send outputs to PM. DO NOT retry this tool.`,
      );
    }
    throw new Error(
      `STOP — "${action}" requires superuser role (you have "${config.role}"). DO NOT retry this tool.`,
    );
  }
}
